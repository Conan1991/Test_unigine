...
void session::turn_on()
{
    ...
    Objects::objects[2]->getNode()->setEnabled(1);
    //Objects::objects[1]->getNode()->grab();
    //Objects::objects[1]->getNode()->release();
    qDebug() << "Object 2 is enabled bfdbfbrtrrbbr";
}
...
void Tthread::run()
{
    ...
    forever{
        ...
        loop.exec();
        if( Objects::objects[3]->getNode()->isEnabled() == 1  )
        {
            do_enable_object(3, 0);
            //do_enable_object(2, 0);
        }
        else if ( Objects::objects[3]->getNode()->isEnabled() == 0  )
        {
            do_enable_object(3, 1);
            //do_enable_object(2, 1);
        }
        qDebug() << "Qthread working"<<  currentThread() << endl; 
    }
}
